...
void Map::AddKeyFrame(KeyFrame *pKF)
{
    ...
    cout << setprecision(6) << pKF->mTimeStamp << setprecision(7) << " " << t(0) << " " << t(1) << " " << t(2)
...
void Map::_WriteMapPoint(ofstream &f, MapPoint *mp, const std::string &end_marker) {
    Eigen::Vector3f wp = mp->GetWorldPos();
    cv::Mat cvMat = (cv::Mat_<float>(3, 1) << wp(0), wp(1), wp(2));

    f << cvMat.at<float>(0, 0) << " "; // pos x: float
    f << cvMat.at<float>(1, 0) << " "; // pos y: float
    f << cvMat.at<float>(2, 0) << end_marker; // pos z: float
}
