...
void* Body::move_thread(void *param) {
    ...
    int pause_counter = 0;
    while(*run_flag) {
        if(leg_num < 0) leg_num = -leg_num + 1;
        ...
        body->pose(
            ...
            0, 
            0, 
            body->legs[leg_num].is_front() ? b_leen_off : f_leen_off, 
            body->legs[leg_num].is_right() ? l_leen_off : r_leen_off, 
            140
        );
        // wait_real(250);
        ...
        body->legs[leg_num].move(temp_vector);
        // wait_real(250);
